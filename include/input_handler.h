...
private:
	static float point_to_segement_distance(float px, float py, float x1, float y1, 
		float x2, float y2) 
	{
		float ABx = x2 - x1;
		float ABy = y2 - y1;
		...
		const std::vector<Constraint>& constraints) 
	{
		...
		{
			float distance = point_to_segement_distance(mouse_x, mouse_y, 
